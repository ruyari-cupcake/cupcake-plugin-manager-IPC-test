// aws-signer.js — Shared: AWS V4 Signer for Bedrock
...
export class AwsV4Signer {
    ...
    constructor({
        ...
        this.canonicalHeaders = this.signableHeaders
            .map((header) => header + ":" + (header === "host"
                ? this.url.host
                : (this.headers.get(header) || "").replace(/\s+/g, " ")))
            .join("\n");

        this.credentialString = [
            this.datetime.slice(0, 8), this.region, this.service, "aws4_request"
        ].join("/");
    ...
    async stringToSign() {
        return [
            "AWS4-HMAC-SHA256", this.datetime, this.credentialString,
    ...
    async canonicalString() {
        return [
            this.method.toUpperCase(), this.encodedPath, this.encodedSearch,
            this.canonicalHeaders + "\n", this.signedHeaders, await this.hexBodyHash()
        ].join("\n");
    }
...
export function buf2hex(arrayBuffer) {
    ...
    let o = "";
    for (let i = 0; i < b.length; i++) { o += HEX[(b[i] >>> 4) & 15]; o += HEX[b[i] & 15]; }
    return o;
}

export function encodeRfc3986(s) {
...
export function guessServiceRegion(url, headers) {
    ...
    if (hostname.endsWith(".on.aws")) {
        const m = hostname.match(/^[^.]{1,63}\.lambda-url\.([^.]{1,63})\.on\.aws$/);
        return m ? ["lambda", m[1] || ""] : ["", ""];
    }
    if (hostname.endsWith(".r2.cloudflarestorage.com")) return ["s3", "auto"];
    if (hostname.endsWith(".backblazeb2.com")) {
        const m = hostname.match(/^(?:[^.]{1,63}\.)?s3\.([^.]{1,63})\.backblazeb2\.com$/);
        return m ? ["s3", m[1] || ""] : ["", ""];
    }
    ...
    if (region === "us-gov") region = "us-gov-west-1";
    else if (region === "s3" || region === "s3-accelerate") { region = "us-east-1"; service = "s3"; }
    else if (service === "iot") {
        ...
        const tp = (headers.get("X-Amz-Target") || "").split(".")[0];
        if (tp === "AnyScaleFrontendService") service = "application-autoscaling";
        else if (tp === "AnyScaleScalingPlannerFrontendService") service = "autoscaling-plans";
